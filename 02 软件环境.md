...
sudo apt install python-dev python-pip libxml2-dev libxslt1-dev zlib1g-dev libffi-dev libssl-dev     (do before 'pip3 install jupyter notebook jupyterlab')
```
- python库安装
```
pip3 install cython
...
sudo make install
```
打开`/etc/ld.so.conf`，末行加入`include /usr/loacal/lib`，运行`sudo ldconfig`
